...
Stage * StageManager::getStage(int stageType, bool isWall[])
{
	...
	case 0: // 일반 방
	{
		...
		else if (!isWall[0] && !isWall[1] && isWall[2] && !isWall[3])//좌, 상, 하  뚫린 경우
		{
			resultRoom = new Room13LTB;    
		}
		else if (isWall[0] && !isWall[1] && !isWall[2] && !isWall[3])// 우, 상, 하  뚫린 경우
		{
		...
		else if (!isWall[0] && isWall[1] && isWall[2] && isWall[3])//좌 뚫린 경우
		{
	...
	case 1: // 시작방
	{
		...
		else if (isWall[0] && isWall[1] && isWall[2] && !isWall[3]) // 하 꿇린 경우
		{
	...
	case 2: // 끝방
	{
		...
		else if (!isWall[0] && !isWall[1] && isWall[2] && isWall[3]) //좌, 상
		{
			resultRoom = new DownStair2LT;

		}
		else if (isWall[0] && !isWall[1] && isWall[2] && isWall[3]) //상
		{
	...
	case 3: // 식당방
	{
		if (!isWall[0] && isWall[1] && isWall[2] && isWall[3]) // 좌가 뚫린 경우
		{
			resultRoom = new RestaurantRoom;
		}
		else if (!isWall[0] && isWall[1] && !isWall[2] && isWall[3]) // 좌, 우가 뚫린 경우
		{
			resultRoom = new RestaurantRoom2LR;
		}
		else if (isWall[0] && isWall[1] && !isWall[2] && isWall[3]) // 우가 뚫린 경우
		{
	...
	case 4: // 상점방
	{
		if (!isWall[0] && isWall[1] && !isWall[2] && isWall[3]) // 좌, 우가 뚫린 경우
		{
			resultRoom = new DungeonShopRoom3;
		}
		else if (!isWall[0] && isWall[1] && isWall[2] && isWall[3]) // 좌가 뚫린 경우
		{
...
bool StageManager::makeSpecialRoom()
{
	...
	for (int x = 0; x < _mapSize; x++)
	{
		for (int y = 0; y < _mapSize; y++)
		{
			...
			else if (!_roomInfo[x][y].isWall[0] && !_roomInfo[x][y].isWall[1] && _roomInfo[x][y].isWall[2] && _roomInfo[x][y].isWall[3]) // 좌, 상
			{
				candidate.push_back(Vector2(x, y));
			}
		}
	...
	for (int x = 0; x < _mapSize; x++)
	{
		for (int y = 0; y < _mapSize; y++)
		{
			...
			else if (!_roomInfo[x][y].isWall[0] && _roomInfo[x][y].isWall[1] && _roomInfo[x][y].isWall[2] && _roomInfo[x][y].isWall[3]) // 좌
			{
				candidate.push_back(Vector2(x, y));
			}
			else if (_roomInfo[x][y].isWall[0] && _roomInfo[x][y].isWall[1] && !_roomInfo[x][y].isWall[2] && _roomInfo[x][y].isWall[3]) // 우
