...
void PickaxeRed::frontRender(Player* player)
{
	...
	float handDegree = degree +((isLeft) ? -190 : 190);
	...
	Vector2 renderPosWeapon = originPos;
		...
		renderPosHand.x += (isLeft) ? ( cosf(weaponDegree * (PI / 180)) * width * 0.15 * 4) : ( -cosf(weaponDegree * (PI / 180)) * width * 0.15 * 4);
	...
	if (_drawEffect) // 이펙트를 그린다
	{
		...
		EFFECT_MANAGER->play("EFFECT_REDPICKAXESWING", effectPos, Vector2(100, 200), degree);
	}
